...
export const getPublicAssetReference = (value) => {
  ...
  if (!isRelative) {
    ...
    if (publicOrigin) allowedHosts.add(new URL(publicOrigin).hostname.toLowerCase());
    for (const candidate of String(process.env.LEGACY_ASSET_ORIGINS || "").split(",")) {
      try {
        if (candidate.trim()) allowedHosts.add(new URL(candidate.trim()).hostname.toLowerCase());
      } catch {
        throw new Error("LEGACY_ASSET_ORIGINS must contain absolute URLs");
      }
    }
