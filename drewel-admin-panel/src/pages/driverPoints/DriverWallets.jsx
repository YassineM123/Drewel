    ...
    <PointsPageShell title="Driver Wallets" description="Search balances and open the immutable wallet ledger." isOwner={access.isOwner}>
      ...
      <section className="tile">
        {state.loading ? <PointsLoading label="Loading driver wallets..." /> : state.error ? <PointsError message={state.error} onRetry={load} /> : !result.drivers.length ? <PointsEmpty title="No wallets found" message="Try changing the search or filters." /> : <>
          <div className="points-table-wrap"><table className="points-table"><thead><tr><th>Driver</th><th>Driver ID</th><th>Verification</th><th>Available</th><th>Reserved</th><th>Purchased</th><th>Bonus</th><th>Rides charged</th><th>Last transaction</th><th>Actions</th></tr></thead><tbody>{result.drivers.map((driver) => <tr key={driver.id}><td><strong>{driver.fullName || "Unnamed driver"}</strong></td><td><code>{driver.id}</code></td><td><span className={`points-badge ${driver.verificationStatus === "verified" ? "points-badge--success" : "points-badge--warning"}`}>{driver.verificationStatus}</span></td><td>{driver.wallet?.availablePoints ?? 0}</td><td>{driver.wallet?.reservedPoints ?? 0}</td><td>{driver.wallet?.availablePurchasedPoints ?? driver.wallet?.purchasedPoints ?? 0}</td><td>{driver.wallet?.availableBonusPoints ?? driver.wallet?.bonusPoints ?? 0}</td><td>{driver.confirmedRidesCharged || 0}</td><td>{driver.lastTransaction ? <>{driver.lastTransaction.type}<br/><small>{new Date(driver.lastTransaction.createdAt).toLocaleString()}</small></> : "None"}</td><td><div className="points-actions"><Link className="btn btn-sm btn-outline-primary" to={`/driver-points/wallets/${driver.id}`}>View wallet</Link>{access.isOwner && <button className="btn btn-sm btn-success" type="button" onClick={() => setAdjustment({ driver, mode: "credit-purchase" })}>Add points</button>}{access.canAdjust && <><button className="btn btn-sm btn-outline-success" type="button" onClick={() => setAdjustment({ driver, mode: "credit-free" })}>Free credit</button><button className="btn btn-sm btn-outline-danger" type="button" onClick={() => setAdjustment({ driver, mode: "debit" })}>Debit/correct</button></>}</div></td></tr>)}</tbody></table></div>
