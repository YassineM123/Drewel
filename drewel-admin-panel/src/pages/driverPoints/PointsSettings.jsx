...
const PointsSettings = () => {
  ...
  return <PointsPageShell title="Settings" description="Owner-only policy values used by new operations." isOwner={isOwner}>
    {state.loading ? <div className="tile"><PointsLoading /></div> : state.error && !settings ? <div className="tile"><PointsError message={state.error} onRetry={load} /></div> : <section className="tile p-4"><div className="points-alert"><strong>Non-retroactive:</strong> changing these values does not rewrite completed transactions or historical balances.</div>{!isOwner && <div className="points-alert">Read-only: only the Drewel Owner can change these settings.</div>}{state.error && <div className="alert alert-danger" role="alert">{state.error}</div>}<div className="points-form">{FIELDS.map(([name, label, min]) => <div key={name}><label htmlFor={`setting-${name}`}>{label}</label><input id={`setting-${name}`} type="number" min={min} step="1" disabled={!isOwner} value={settings?.[name] ?? ""} onChange={(event) => setSettings({ ...settings, [name]: event.target.value })} /></div>)}{FLOAT_FIELDS.map(([name, label, min]) => <div key={name}><label htmlFor={`setting-${name}`}>{label}</label><input id={`setting-${name}`} type="number" min={min} step="0.01" disabled={!isOwner} value={settings?.[name] ?? ""} onChange={(event) => setSettings({ ...settings, [name]: event.target.value })} /></div>)}<div className="points-form__wide"><label htmlFor="settings-reason">Reason for change</label><textarea id="settings-reason" rows="3" disabled={!isOwner} value={reason} onChange={(event) => setReason(event.target.value)} /></div></div>{isOwner && <div className="mt-3"><button className="btn btn-primary" type="button" disabled={invalid} onClick={() => setConfirming(true)}>Review settings update</button></div>}</section>}
