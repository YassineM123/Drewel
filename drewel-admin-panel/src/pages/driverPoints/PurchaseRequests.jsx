...
const PurchaseRequests = () => {
  ...
  const perform = async () => {
    ...
    try {
      if (action.status === "credited") await creditPurchaseRequest(action.request._id, { reason: form.reason || "Verified point purchase credited" }, key);
  ...
  const invalid = terminal && form.reason.trim().length < 3 || payment && (!form.paymentReference.trim() || !form.paymentAmount || form.currency.trim().length !== 3 || form.paymentMethod.trim().length < 2);
