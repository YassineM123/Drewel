...
const DriverWalletDetail = () => {
  ...
  useEffect(() => { load(); }, [load]);
  const actions = <div className="points-actions"><Link className="btn btn-light" to="/points/balances">Back</Link>{access.canAdjust && data?.driver && <><button className="btn btn-success" type="button" onClick={() => setMode("credit-free")}>Add system points</button><button className="btn btn-outline-danger" type="button" onClick={() => setMode("debit")}>Debit/correct</button></>}</div>;
  return (
