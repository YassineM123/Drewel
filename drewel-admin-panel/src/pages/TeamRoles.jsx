...
import { getTeam, getRolesCatalog, rolesErrorMessage } from "../api/domains/roles";
import {
  Card, Button, SectionHeader, Th, Td, Tr, Avatar, Badge,
  Modal, Toast, StatRow, LoadingState, ErrorState, EmptyState, Input, Select
} from "../components/ui";
...
const TeamRoles = () => {
  ...
  const load = useCallback(async (signal) => {
    try {
      setLoading(true);
      setError("");
      const [teamResult, rolesResult] = await Promise.all([
        getTeam({}, signal),
        getRolesCatalog(signal),
      ]);
      setTeam(teamResult.admins || []);
  ...
  const roleCounts = useMemo(() => {
    const counts = { owner: 0, admin: 0, support: 0, analyst: 0 };
    team.forEach((m) => {
      const role = (m.role || "analyst").toLowerCase();
  ...
  const getInitials = (member) => {
    const name = member.name || member.fullName || `${member.firstName || ""} ${member.lastName || ""}`.trim();
    if (!name) return "??";
    return name.split(" ").map((w) => w[0]).join("").toUpperCase().slice(0, 2);
  };

  const getMemberName = (member) =>
    member.name || member.fullName || `${member.firstName || ""} ${member.lastName || ""}`.trim() || "Unknown";
      ...
      {activeTab === "members" && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {["owner", "admin", "support", "analyst"].map((role) => {
                        ...
                        <Tr key={member._id || member.id}>
                          <Td>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-semibold text-slate-800">{getMemberName(member)}</p>
                                <p className="text-xs text-slate-400 flex items-center gap-1">
                                  <Mail size={10} />{getMemberEmail(member)}
                                </p>
                              </div>
                            </div>
                          </Td>
                          ...
                          <Td><span className="text-xs text-slate-500">{member.lastActive ? relTime(member.lastActive) : "—"}</span></Td>
                          <Td><span className="text-xs text-slate-500">{member.addedAt ? formatDate(member.addedAt) : member.createdAt ? formatDate(member.createdAt) : "—"}</span></Td>
                          <Td>
            ...
            <table className="w-full">
              <thead>
                <tr>
                  ...
                  {["owner", "admin", "support", "analyst"].map((r) => (
      ...
      <Modal open={inviteOpen} onClose={() => setInviteOpen(false)} title="Invite Team Member">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-10 bg-white border border-slate-200 rounded-[10px] pl-9 pr-4 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#BE1B2C]/20 focus:border-[#BE1B2C]/40 transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button variant="primary" disabled={!inviteEmail.trim()} loading={inviteLoading} onClick={handleInvite} icon={<Mail size={14} />}>
              Send Invitation
            </Button>
          </div>
        </div>
