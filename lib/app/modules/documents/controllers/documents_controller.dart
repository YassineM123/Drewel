...
class DocumentsController extends GetxController {
  ...
  List<Map<String, dynamic>> fileNameList = [
    {'name': 'Car License - Front', 'key': ApiKeyConstants.carLicenseFront, 'requiresBack': true, 'isBack': false},
    {'name': 'Car License - Back', 'key': ApiKeyConstants.carLicenseBack, 'requiresBack': false, 'isBack': true},
    {'name': 'Driver License - Front', 'key': ApiKeyConstants.drivingLicenseFront, 'requiresBack': true, 'isBack': false},
    {'name': 'Driver License - Back', 'key': ApiKeyConstants.drivingLicenseBack, 'requiresBack': false, 'isBack': true},
    {'name': 'ID Proof - Front', 'key': ApiKeyConstants.idProofFront, 'requiresBack': true, 'isBack': false},
    {'name': 'ID Proof - Back', 'key': ApiKeyConstants.idProofBack, 'requiresBack': false, 'isBack': true},
    {'name': 'Profile Image', 'key': ApiKeyConstants.profileImage, 'requiresBack': false, 'isBack': false},
    {'name': 'Passport Copy', 'key': ApiKeyConstants.passportCopy, 'requiresBack': false, 'isBack': false},
    {'name': 'License Company', 'key': ApiKeyConstants.licenseCompany, 'requiresBack': false, 'isBack': false},
  ];
  @override
  void onInit() async{
    super.onInit();
    ...
    try {
      ...
      if (loginModel != null && loginModel.success != null &&
        ...
        if (whatsapp.startsWith('+')) {
          ...
          whatsappCountryCode.value = driverDetail?.driver?.countryCode ?? '+971';
        }
        ...
        documentUrl[0] = driverDetail?.driver?.carLicenseFrontUrl ?? driverDetail?.driver?.carLicenseUrl ?? '';
        documentUrl[1] = driverDetail?.driver?.carLicenseBackUrl ?? '';
        documentUrl[2] = driverDetail?.driver?.drivingLicenseFrontUrl ?? driverDetail?.driver?.drivingLicenseUrl ?? '';
        documentUrl[3] = driverDetail?.driver?.drivingLicenseBackUrl ?? '';
        documentUrl[4] = driverDetail?.driver?.idProofFrontUrl ?? driverDetail?.driver?.idProofUrl ?? '';
  ...
  bool _fieldEquals(String? a, String? b) => (a ?? '').trim() == (b ?? '').trim();
  ...
  void _evaluatePendingApprovalStatus() {
    ...
    if (!_fieldEquals(driver.drivingLicenseFrontUrl, logs.drivingLicenseFrontUrl) ||
        !_fieldEquals(driver.drivingLicenseBackUrl, logs.drivingLicenseBackUrl)) {
    ...
    hasPendingApproval.value = (!mainApproved || !logsApproved) && items.isNotEmpty;
  ...
  Future<void> clickOnSubmit(BuildContext context) async {
    if (cityController.text.isNotEmpty && typeController.text.isNotEmpty) {
      ...
      if (selectedFile[0] != null && selectedFile[1] == null && documentUrl[1].isEmpty) {
        pairedDocsValid = false;
        missingBackDocs.add('Car License Back');
      }
      
      // Driver License pair (index 2 = front, index 3 = back)
      if (selectedFile[2] != null && selectedFile[3] == null && documentUrl[3].isEmpty) {
        pairedDocsValid = false;
        missingBackDocs.add('Driver License Back');
      }
      
      // ID Proof pair (index 4 = front, index 5 = back)
      if (selectedFile[4] != null && selectedFile[5] == null && documentUrl[5].isEmpty) {
        pairedDocsValid = false;
        missingBackDocs.add('ID Proof Back');
      }
      
      if (!pairedDocsValid) {
        CommonWidgets.snackBarView(title: 'Please upload: ${missingBackDocs.join(', ')}');
        return;
      }
      ...
      try {
        ...
        String driverId = sp.getString(ApiKeyConstants.userId) ?? driverDetail?.driver?.sId ?? '';
        
        // Get phone number without country code
        String phoneNumber = sp.getString(ApiKeyConstants.phone) ?? driverDetail?.driver?.phone ?? '';
        ...
        await ApiMethods.driverUpdateDetailsApi(bodyParams: bodyParams, imageList: imagesToSend, imageKeyList: imageKeysToSend);
        
        // Debug: Print server response
        print('=== SERVER RESPONSE ===');
        print('Success: ${addDriverDetailModel?.success}');
        print('Message: ${addDriverDetailModel?.message}');
        print('Driver vehicle type from response: ${addDriverDetailModel?.driver?.vehicleType}');
        print('========================');
        
        if (addDriverDetailModel != null && addDriverDetailModel.success != null &&
            addDriverDetailModel.success! && addDriverDetailModel.driver!=null ) {
          ...
          CommonWidgets.snackBarView(
            title: 'Your updated documents have been sent to the admin for approval.',
            success: true,
          );

        } else {
          CommonWidgets.snackBarView(title: addDriverDetailModel?.message??'Driver Details Failed ...');
        }
      } catch (e) {
      ...
      if (Get.isRegistered<DriverHomeController>()) {
        ...
        if (driverDetail?.driver != null) {
          driverHomeController.userData.value = {
            ...
            ApiKeyConstants.countryCode: driverDetail!.driver!.countryCode ?? '',
            ApiKeyConstants.profileImage: driverDetail!.driver!.profileImageUrl ?? '',
            ApiKeyConstants.fullName: driverDetail!.driver!.fullName ?? '',
      ...
      builder: (BuildContext context) {
        return Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                Center(child: Text(StringConstants.selectCity,style: MyTextStyle.titleStyle18bb,)),
                ListView.builder(
                    shrinkWrap: true,
                    ...
                    itemBuilder: (context,index){
                      ...
                      return GestureDetector(
                        onTap: (){
                          cityController.text=LocalData().cityList[index].replaceAll('\n', '').trim();
                          Get.back();
                        },
                        child: Container(
                          ...
                          child:Text(LocalData().cityList[index].replaceAll('\n', '').trim(),
      ...
      builder: (BuildContext context) {
        return Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                Center(child: Text(StringConstants.selectvehicleType,style: MyTextStyle.titleStyle18bb,)),
                ListView.builder(
                    shrinkWrap: true,
                    ...
                    itemBuilder: (context,index){
                      ...
                      return GestureDetector(
                        onTap: (){
                          typeController.text=LocalData().transportList[index]['name'].toString().trim();
                          print('Selected vehicle type: ${typeController.text}');
                          Get.back();
                        ...
                        child: Container(
                          ...
                          child:Row(
                            children: [
                              CommonWidgets.appIcons(assetName:LocalData().transportList[index]['image']??'',
                                  height: 35.px,width: 35.px,
                                  color: Colors.black87),
                              SizedBox(width: 10.px,),
                              Text(LocalData().transportList[index]['name'].toString(),
                                style: MyTextStyle.titleStyle16bb,
                              ),
                            ],
                          ) ,
